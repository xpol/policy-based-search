...
namespace gg
{
	namespace Detail
	{
		...
		template <typename Graph, typename WeightGenerator, typename SizeType>
		void construct(Graph &g, SizeType const N, size_t const B, WeightGenerator &weight_generator)
		{
	...
	template <typename Graph, typename Engine, typename SizeType>
	void generate_graph(Graph &g, SizeType const N, unsigned const B, Engine engine)
	{
		typedef typename boost::property_traits<typename boost::property_map<Graph, boost::edge_weight_t>::type>::value_type Weight;
		
		Detail::check_preconditions(N, B);
		...
		// Do the actual work.
		Detail::construct(g, N, B, weight_generator);
		assert(Detail::correct(g, B));
