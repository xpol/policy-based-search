...
{
	typedef typename Engine::result_type ResultType;
	check_preconditions(boost::num_vertices(g), B);

	typename std::conditional<std::is_integral<ResultType>::value, std::uniform_int_distribution<ResultType>, std::uniform_real_distribution<ResultType>>::type weight_dist(1, 500);
	auto weight_generator(std::bind(weight_dist, engine));

	// Do the actual work.
	construct(g, B, weight_generator);
